...
void NMEA_GPRMC_Analysis(nmea_msg *gpsx,u8 *buf)
{
  ...
  if(posx!=0XFF)
  {
    ...
    gpsx->utc.ms=temp%NMEA_Pow(10,dx);      
...
{       
  ...
  if(len<250)
  {
    for(i=0;i<UART4_MAX_RECV_LEN;i++)if(UART4_RX_BUF[i]==0XB5)break;//查找同步字符 0XB5
    if(i==UART4_MAX_RECV_LEN)rval=2;            //没有找到同步字符
    else if(UART4_RX_BUF[i+3]==0X00)rval=3;//接收到NACK应答
    else rval=0;                 //接收到ACK应答
  }
  else
    rval=1;                //接收超时错误
  gps_uart_flag=0;              //清除接收
  return rval;  
}
//配置保存
//将当前配置保存在外部EEPROM里面
...
{
  u8 i;
  _ublox_cfg_cfg *cfg_cfg=(_ublox_cfg_cfg *)UART4_TX_BUF;
...
{
  _ublox_cfg_msg *cfg_msg=(_ublox_cfg_msg *)UART4_TX_BUF;
...
{
  _ublox_cfg_prt *cfg_prt=(_ublox_cfg_prt *)UART4_TX_BUF;
...
{
  _ublox_cfg_tp *cfg_tp=(_ublox_cfg_tp *)UART4_TX_BUF;
...
{
  _ublox_cfg_rate *cfg_rate=(_ublox_cfg_rate *)UART4_TX_BUF;
