...
static uint16_t  mtu;
static char *AT_cmd;
static uint16_t rfcomm_hsp_cid = 0;
...
void AT(char * str)
{
  ...
  for(i = 0; i <= len; i++)
    rt_kprintf("%c",str[i]);
  rfcomm_send_internal(rfcomm_channel_id,str, len + 1);

}
#include <finsh.h>
FINSH_FUNCTION_EXPORT(AT,AT cmd)
					...
					case RFCOMM_EVENT_PORT_CONFIGURATION:
						{
							...
							log_info("RFCOMM channel configuration send.");
...
void hsp_rfcomm_packet_handler (void * connection, uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size){
